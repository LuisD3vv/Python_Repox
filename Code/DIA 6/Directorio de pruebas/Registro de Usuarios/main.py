...
def registrar_usuario(usuario, contra):
    contra_bytes = contra.encode() # convertir a Bites
    salt = bcrypt.gensalt() # Generar un valor aleatorio
    encript = bcrypt.hashpw(contra_bytes,salt) # une el valor aleatorio junto con la contraseña codificada
    fecha = datetime.now().strftime("%Y-%m-%d %H:%M:%S") # transfromar en string el objeto date
...
def obtener_hash(usuario):
      with open(ruta, "r") as lectura:
            for linea in lectura:
                if linea == "":
                    continue  
                nombre, hash_pssw = linea.rstrip("user")
                if nombre == usuario:
                    return hash_pssw
...
def log_usuario(usuario, contrasenia):
    encript = obtener_hash(usuario)
    print(encript)
    if encript is None:
        print("Usuario no encontrado")
        return

        if bcrypt.checkpw(contrasenia.encode(), encript):
...
while True:
    ...
    if opcion == 1:
        ...
        log_usuario(usuario_login,contrasenia_login)
