...
# Directorio general
directorios = {
    "Carnes": Path(ruta, Path("Carnes")),
    "Ensaladas": Path(ruta, Path("Ensaladas")),
    "Pastas": Path(ruta, Path("Pastas")),
    "Postres": Path(ruta) / "Postres",
}
...
while True:
    ...
    if opcion == 3:
        ...
        #verificar si la categoria no existe
        try:
            Categoria_nueva.mkdir() # Crearla si no existe
            print("Carpeta creada exitosamente")
        except FileExistsError: # si ya existe arroja esto
            print("La carpeta ya existe")
        except Exception as e: # cualquier otro error arroja error
