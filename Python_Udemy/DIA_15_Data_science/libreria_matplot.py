...
Original file is located at
    https://colab.research.google.com/drive/   11WLMO1b1thTBQcPS-4pkvR1E72RAbY09
"""
...
ax.set(title="Grafico de casos de covid-19 en Mexico",xlabel="dias",ylabel="defunciones")
# Guardamos nuestro gráfico empleando fig.savefig()
fig.savefig('/ejemplo-grafico.png')
...
x_1 = np.linspace(0,100,20) # crear un array, pero en este caso se usa para crear la curva
y_1 = x_1**2
...
ax.set(title="Precios",xlabel="comidas",ylabel="Precios")

# Probemos a continuación con un gráfico de barras horizontales
fig, ax = plt.subplots()
ax.barh(list(comidas.keys()),list(comidas.values()))
ax.set(title="Histograma de precio/comidas",ylabel="comidas",xlabel="Precios")
...
# Para nuestro primer gráfico, tomamos el conjunto x_1, y_1, y generamos un gráfico de líneas
ax1.plot(x_1,y_1,color="#222")

# Para nuestro segundo gráfico, tomamos el conjunto x_2, y_2, y generamos un gráfico de dispersión
ax2.scatter(x_2,y_2,color="#222")

# Creamos un gráfico con los precios de tres comidas en la esquina inferior izquierda
ax3.bar(comidas.keys(),comidas.values(),color="#222")

# El gráfico de la esquina inferior derecha será un histograma de valores aleatorios con distribución normal
ax4.hist(np.random.randn(1000),color="#222")